...
def eval(x, env=global_env):
    ...
    if isa(x, Symbol):
        ...
        # err...maybe not? It returns a <built-in function ___> object

        # >>> global_env['*']
        # <built-in function mul>
        # >>> global_env.find('*') returns the global_env dictionary. 
        # So we could just replace env.find(x)[x] with env[x] for the same result...right?
        # the .find is simply to check if x exists in the dictionary...

        # Okay, you are retarded. .find is a function defined in the Env class here. 
    ...
    # procedure (lambda (var*) exp)
        ...
        # new_env_dict = dict(env.items() + vars.items() + args.items())
        # updated_env = Env(new_env_dict)
        # return lambda *args: eval(exp, Env(dict(env.items() + vars.items() + args.items())))
        # HOW CAN I SPLIT THIS UP. I DON'T KNOW HOW TO ACCECSS ARGS OTHERWISE.
